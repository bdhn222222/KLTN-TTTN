...
const ScheduleDoctor = () => {
    ...
    try {
      ...
      if (response.data && response.data.success && response.data.data) {
        ...
        const formattedDayOffs = response.data.data.map((dayOff) => {
          ...
          const affectedAppointments = (dayOff.affected_appointments || []).map(
            (apt) => ({
              ...apt,
              patient_name:
                apt.patient_name ||
        ...
        console.warn(
          "Không có dữ liệu ngày nghỉ hoặc cấu trúc phản hồi không đúng:",
          response.data
        );
        setDayOffs([]);
      }
  ...
  const [isAppointmentsModalVisible, setIsAppointmentsModalVisible] =
    useState(false);
  const [selectedDayOff, setSelectedDayOff] = useState(null);
  const [appointmentModalTitle, setAppointmentModalTitle] = useState(
    "Danh sách cuộc hẹn bị ảnh hưởng"
  );

  const showAffectedAppointments = (record) => {
    setSelectedDayOff(record);
    setAppointmentModalTitle(
      `Cuộc hẹn bị ảnh hưởng - Ngày ${dayjs(record.date).format("DD/MM/YYYY")}`
    );
    ...
    if (
      ...
      showNotification(
        "info",
        "Thông tin",
        "Không có cuộc hẹn nào bị ảnh hưởng"
      );
  ...
  const showCancelAffectedAppointments = (appointments, dayOff) => {
    if (appointments && appointments.length > 0) {
      ...
      setAffectedAppointments(processedAppointments);
      setAppointmentModalTitle(
        `Cuộc hẹn đã được khôi phục - Ngày ${dayjs(dayOff.off_date).format(
          "DD/MM/YYYY"
        )}`
      );
      setIsAppointmentsModalVisible(true);
          ...
          >
            <Radio.Group>
              ...
              <Radio value="afternoon">
                Buổi chiều ({scheduleHours.afternoon})
              </Radio>
