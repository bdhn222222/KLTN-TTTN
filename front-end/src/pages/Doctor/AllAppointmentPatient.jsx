...
const AllAppointmentPatient = () => {
  ...
  const family_member_id = patient_id; // Maintaining backward compatibility
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const response = await axios.get(
        `${url1}/doctor/patients/${family_member_id}/appointments`,
        {
          headers: {
          ...
          <Content style={{ margin: "24px 16px", overflow: "initial" }}>
            ...
            <Card className="mb-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <Title level={4}>
                    {patientInfo.name || patientInfo.username}
                  </Title>
                  {patientInfo.phone_number && (
                    <div>SĐT: {patientInfo.phone_number}</div>
                  )}
                  {patientInfo.email && <div>Email: {patientInfo.email}</div>}
                  <div>ID: {family_member_id}</div>
                </div>
                <div className="ml-auto">
                  <Button
                    ...
                    onClick={() =>
                      navigate(`/doctor/patients/${family_member_id}/detail`)
                    }
