...
const PaymentPaidAdmin = () => {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      if (response.data && response.data.data) {
        ...
        const withPaidPayment = response.data.data.filter((appt) => {
        ...
        console.log("Filtered appointments with payments:", withPaidPayment);
        setAppointments(withPaidPayment);
      } else {
  ...
  return (
    <>
      {contextHolder}
      <Card title="Danh sách cuộc hẹn đã thanh toán">
        <Table
          columns={columns}
          ...
          locale={{
            emptyText: (
              <Empty description="Không có cuộc hẹn nào đã thanh toán" />
            ),
          }}
          pagination={{
            pageSize: 10,
...
export default PaymentPaidAdmin;
